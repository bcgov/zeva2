...
const formattedAddress = (address: OrganizationAddress | undefined) => {
  ...
  const { addressLines, city, state, postalCode, country, representative } = address;
  return (
    <div>
      {representative && <p>{representative}</p>}
      {addressLines && addressLines.split("\n").map((line, index) => (<p key={index}>{line}</p>))}
      {(city || state || postalCode) && <p>{city}, {state} {postalCode}</p>}
      {country && <p>{country}</p>}
    </div>
  )
};
...
const organizationUsers = (users: OrganizationUser[]) => {
  ...
  return (
    <ul>
      {users.map((user) => (
        <li key={user.id}>
          <a href={`/users/${user.id}`} className="text-primaryBlue hover:underline">
            {user.firstName} {user.lastName} [{user.roles.map(role => enumToTitleString(role)).join(" | ")}]
          </a>
        </li>
      ))}
    </ul>
