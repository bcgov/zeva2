...
export const AgreementHistories = (props: {
  ...
  const [histories, setHistories] = useState<AgreementHistoryType[]>(props.agreementHistory);
  ...
  return (
    <div className={props.className}>
      {histories.map((history) => {
        ...
        return (history.userAction === AgreementUserAction.ADDED_COMMENT_GOV_INTERNAL) ? (
          <div key={`history-${history.id}`}>
            <span className="font-semibold">Comment</span>
            {` by ${userName} [${history.timestamp.toLocaleString()}]`}
            {history.agreementComment?.comment.split("\n").map((paragraph, index) => (
              <p key={`history-${history.id}-line-${index}`} className="pl-4 text-primaryBlue">
                {paragraph}
              </p>
            ))}
          </div>
      ...
      {!isProcessing && <Button
        ...
        onClick={async () => {
          ...
          const createdHistory = await props.handleAddComment(newComment.trim());
