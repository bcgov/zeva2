...
export const AttachmentsList = (props: {
  ...
  const [deletingAttachmentObj, setDeletingAttachmentObj] = useState<string | null>(null);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  const handleDeleteAttachment = useCallback(async (objectName: string) => {
  ...
  return (
    <div className={props.className}>
      {files.length === 0 && <p className="text-sm text-gray-500">No attachments</p>}
      <ul className="list-disc list-inside text-sm text-gray-600">
        {files.map((attachment) => (
          <li key={attachment.objectName}>
            {attachment.fileName}
            {props.deleteAttachment && <Button 
            ...
            >
              {deletingAttachmentObj === attachment.objectName ? "..." : "Delete"}
            </Button>}
          </li>
        ))}
      </ul>
